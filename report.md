...
> **Last Updated:** 2026-06-27 (batch 23 — Factual alignment with codebase; Hero standardization)
...
| Date | Change |
|------|--------|
| 2026-06-27 | **Batch 24 — CDN Vendoring + Fallback** — Downloaded all 6 CDN JS dependencies to `js/vendor/`: three.js (590KB), GSAP (70KB), ScrollTrigger (42KB), ScrollToPlugin (4KB), D3 (274KB), TopoJSON (21KB). Added `onerror` fallback handlers to every CDN `<script>` tag across all 8 HTML pages — if a CDN fails to load, the page dynamically creates a `<script>` pointing to the local vendor copy. Added `js/vendor/` to `.gitignore`. Updated `report.md` file structure. |
| 2026-06-27 | **Batch 23 — Hero Section Standardization** — Unified hero layout, typography, spacing, and CTA alignment across all 8 pages using the Cyber Security hero as the reference standard. **Headline:** standardized to `font-weight: 800`, `font-size: clamp(40px, 5.5vw, 72px)`, `line-height: 1.02`, `letter-spacing: -0.025em` on all pages. **Eyebrow:** standardized to DM Mono, `font-size: clamp(10px, 1.1vw, 13px)`, `letter-spacing: 0.18em`, `color: var(--accent)`, `margin-bottom: 20px`. **Body:** standardized to `font-family: var(--fd)`, `font-size: clamp(14px, 1.4vw, 17px)`, `line-height: 1.65`, `max-width: 480px`, `margin-bottom: 36px`. **CTA:** standardized to `display: flex; gap: 16px; align-items: center; flex-wrap: wrap`. **Responsive:** added 900px stacking breakpoint (panel moves above copy) and 1024px headline clamp (`clamp(36px, 5vw, 60px)`) to all subpage heroes. Affected files: `hero.css`, `hero-msp.css`, `hero-devops.css`, `hero-itsupport.css`, `hero-staffaug.css`, `hero-ai.css`, `about.css`. No JS, animation, or ScrollTrigger files were touched. |
...
│
├── vendor/                          ← Local CDN fallback files [NEW]
│   ├── three.min.js                ← Three.js r128 local copy
│   ├── gsap.min.js                 ← GSAP 3.12.2 local copy
│   ├── ScrollTrigger.min.js        ← GSAP ScrollTrigger local copy
│   ├── ScrollToPlugin.min.js       ← GSAP ScrollToPlugin local copy
│   ├── d3.min.js                   ← D3 7.8.5 local copy
│   └── topojson.min.js             ← TopoJSON 3.0.2 local copy
│
├── .claude/
